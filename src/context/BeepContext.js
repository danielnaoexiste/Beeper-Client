...
const getPosts = (dispatch) => async () => {
  ...
  const response = await beeperApi.get("/posts", {
...
const addPost = (dispatch) => async (title, content, lastEdited, callback) => {
  let token = await AsyncStorage.getItem("token");
  await beeperApi.post(
    "/posts",
    { title, content, lastEdited },
    { headers: { Authorization: "Bearer " + token } }
  );
...
const deletePost = (dispatch) => async (id) => {
  let token = await AsyncStorage.getItem("token");
  await beeperApi.delete(`/remove/${id}`, {
    headers: { Authorization: "Bearer " + token },
  });
  dispatch({ type: "delete_post", payload: id });
};

const editPost = (dispatch) => async (
  id,
  title,
  content,
  lastEdited,
...
const getStars = (dispatch) => async () => {
  let token = await AsyncStorage.getItem("token");
  const response = await beeperApi.get("/starred", {
    headers: { Authorization: "Bearer " + token },
  });
  dispatch({ type: "get_posts", payload: response.data });
};
